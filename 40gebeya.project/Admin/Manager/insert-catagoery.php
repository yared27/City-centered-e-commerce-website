...
<div class="search-cont">
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
      ...
      if ($action == "add") {
          ...
          // Proceed with deleting the selected category
          if ($category !== null) {
              ...
              echo "Please select a category to delete.";
          }
      } elseif ($action == "edit") {
          // Proceed with editing the selected category
          if ($category !== null) {
              $newName = $_POST['newName'];
  
              $sql = "UPDATE category SET categoryName = ? WHERE id = ?";
              $stmt = $conn->prepare($sql);
              $stmt->bind_param("si", $newName, $category);
              $stmt->execute();
              $stmt->close();
          } else {
              echo "Please select a category to edit.";
          }
      }
  }
  ?>
  ...
  <div style="float: right; width: 80%;">
    ...
    <h2 id="delete">Delete Category</h2>
