...
if(strlen($_SESSION['login_user2'])==0)
{   
header('location: customerlogin.php');
}
else{
    if(isset($_POST['update']))
    {
        ...
        $query=mysqli_query($conn,"update customer set fullname='$fullname',contact='$contact' where username='".$_SESSION['login_user2']."'");
        if($query)
        {
...
{
    $sql=mysqli_query($conn,"SELECT password FROM customer WHERE password='".$_POST['cpass']."' AND username='".$_SESSION['login_user2']."'");
    $num=mysqli_fetch_array($sql);
    if($num>0)
    {
        $conn=mysqli_query($conn,"UPDATE customer SET password='".$_POST['newpass']."' WHERE username='".$_SESSION['login_user2']."'");
...
<nav class="navbar navbar-inverse navbar-fixed-top navigation-clean-search" role="navigation">
    <div class="container">
        ...
        $query = "SELECT * FROM customer WHERE username = '{$_SESSION['login_user2']}'";
        $conn = mysqli_connect("localhost", "root", "", "40gebeya");
...
if ($result) {
    ...
    // Now you can use $row['column_name'] to access your data
} else {
